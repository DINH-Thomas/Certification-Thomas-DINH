...
def lr_predict(model, vectorizer, text: str, preprocess_fn=preprocess_text) -> dict:
    ...
    return {"label": int(probability >= 0.5), "probability": float(probability)}
...
def distilbert_predict(model, text: str, tokenizer=None) -> dict:
    """Predict class label/probability with a fine-tuned DistilBERT classifier."""
    return _transformer_predict(model, text, str(config.DISTILBERT_MODEL_HF_PATH), tokenizer)


def mental_roberta_predict(model, text: str, tokenizer=None) -> dict:
    """Predict class label/probability with a fine-tuned mental/mental-roberta-base classifier."""
    return _transformer_predict(model, text, str(config.MENTAL_ROBERTA_HF_PATH), tokenizer)
